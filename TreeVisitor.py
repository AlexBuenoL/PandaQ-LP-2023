...
class TreeVisitor(pandaQVisitor):
    ...
    def visitQuery(self, ctx):
        ...
        # es visiten els camps de la taula 'data' (o nous camps calculats) que l'usuari vol consultar i 
        # es van afegint a la taula 'new_data'. Com que 'data' ja esta ordenada i filtrada,
    ...
    def visitCompSQ(self, ctx):
        ...
        try:
            ...
            self.data = self.data.loc[self.data[col].isin(res_subconsulta)] # es filtra 'data' segons el resultat de la subconsulta
        except KeyError:
            st.error("Error en el where: no s'ha trobat la columna especificada en el where en la taula.")
            self.error = True
